...
<!-- ######################################################################## -->
   ...
   <xsl:template match="article">
      ...
      <xsl:call-template name="lang-attribute-check"/>
		<xsl:if test="$stream='manuscript'">
  ...
  <!-- ********************************************* -->
   ...
   <!-- Match: journal-id
        ...
        2) There must be only one journal-id with a given @journal-id-type
     -->
   <!-- *********************************************************** -->
	<xsl:template match="journal-id">
		<xsl:call-template name="journal-id-check"/>
                <xsl:call-template name="dup-id-type-check">
                   <xsl:with-param name="att-name" select="'journal-id-type'"/>
                </xsl:call-template>
		<xsl:apply-templates select="." mode="output"/>
		</xsl:template>
...
<!--         <xsl:message>
   ...
   <!-- BECK - turning off mathml-subsup-fence-check
      ...
      <xsl:if test="local-name(.) = 'msub'
         or local-name(.) = 'msup'
         or local-name(.) = 'msubsup'
         or local-name(.) = 'munder'
         or local-name(.) = 'mover'
         or local-name(.) = 'munderover'">
         <xsl:call-template name="mml-empty-base-or-script-check">
            <xsl:with-param name="name" select="local-name(.)"/>
         </xsl:call-template>
      </xsl:if>
   ...
   <!-- Match: pub-id
	...
	2) if contains pmid, must be right format
        ...
        6) If @custom-type is present, value of @pub-id-type must be "custom";
           if @pub-id-type="custom", @custom-type must be present and non-empty
        7) There must be only one pub-id with a given @pub-id-type
     -->
   <!-- *********************************************************** -->
   <xsl:template match="pub-id">
	...
	<xsl:call-template name="pmid-check">
      ...
      <xsl:call-template name="dup-id-type-check">
         <xsl:with-param name="att-name" select="'pub-id-type'"/>
      </xsl:call-template>
  ...
  <!-- *********************************************************** -->
   ...
   <!-- *********************************************************** -->
   <!-- Match: @xml:lang
        1) Must be a valid ISO 639-1 (two letter) or ISO 639-2 (three
           letter) language code.
           This test does not look at script or region values.
     -->
   <!-- *********************************************************** -->
   <xsl:template match="@xml:lang">
      ...
      <xsl:variable name="langlist" select="document('stylecheck-helper-templates.xsl')/*/pmcvar:langs"/>
      <xsl:if test="(string-length($xmllang) = 2 and not($langlist/l[@two = $xmllang]))
                    or (string-length($xmllang) = 3 and not($langlist/l[@three = $xmllang]) or $langlist/l[@three = $xmllang]/@two != '')">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'xml_lang'"/>
            <xsl:with-param name="description">
               <xsl:choose>
                  <xsl:when test="string-length($xmllang) = 3 and $langlist/l[@three = $xmllang]/@two != ''">
                     <xsl:text>Three-letter code found: '</xsl:text><xsl:value-of select="."/>
                     <xsl:text>'. Please use its two-letter (ISO 639-1) equivalent: '</xsl:text>
                     <xsl:value-of select="$langlist/l[@three = $xmllang]/@two"/><xsl:text>'.</xsl:text>
                  </xsl:when>
                  <xsl:otherwise> 
                     <xsl:text>Unsupported xml:lang attribute value: '</xsl:text>
                     <xsl:value-of select="."/>
                     <xsl:text>'.</xsl:text>
                     <xsl:text> A complete list of supported values is available at </xsl:text>
                     <xsl:text>https://www.loc.gov/standards/iso639-2/php/code_list.php </xsl:text>
                     <xsl:text>(see the columns 'ISO 639-1 Code' and 'ISO 639-2 Code'). </xsl:text>
                     <xsl:text>For languages that have both a two-letter and three-letter code, </xsl:text>
                     <xsl:text>use the two-letter code.</xsl:text>
                  </xsl:otherwise>
               </xsl:choose>
            </xsl:with-param>
         </xsl:call-template>
