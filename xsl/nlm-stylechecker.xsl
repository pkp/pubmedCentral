...
<!--                                     NLM STYLECHECKER
    ...
    PMC style as defined in the Tagging Guidelines located at:
     ...
     For Manuscripts:
      https://pmc.ncbi.nlm.nih.gov/tagging-guidelines/manuscript/style/
        
    For Books
      https://pmc.ncbi.nlm.nih.gov/tagging-guidelines/book/style/
   ...
   August 11, 2026
        Cut release branch for Version 5.46.
        Public release of 3rd quarter changes will occur on September 9, 2025.

   August 5, 2025
        Updated alternatives-unique-att-values and alternatives-xml-lang-check to
        allow duplicate @xml:lang when differentiated by @specific-use.

        Updated collab-alternatives match template to call "empty-element-check";
        removed call to "href-content-check".

   June 11, 2025
        Updated doi-format-test to translate encoded space to regular space.

   June 10, 2025: Version 5.45
        Public release of 2nd quarter changes.

   June 9, 2025
        Added name-content-check to string-name template.

   May 12, 2025
        Cut release branch for Version 5.45.
        Public release of 2nd quarter changes will occur on June 10, 2025.

   April 17, 2025
        Consolidated caption-format-test into block-formatting-check;
        removed call to caption-format-test.

   April 10, 2025
        Updated @xml:lang check to allow three-letter language codes if
        a two-letter version does not exist.

   March 11, 2025: Version 5.44
        Public release of 1st quarter changes.

   February 18, 2025
        Cut release branch for Version 5.44.
        Public release of 1st quarter changes will occur on March 11, 2025.

   January 16, 2025
        Disallowed all JATS 1.4 lang-* attributes.

   December 10, 2024: Version 5.43
        Public release of 4th quarter changes.

   November 13, 2024
        Reduced severity of mml-empty-base-or-scripts-check from Error to Warning.

        Cut release branch for Version 5.43.
        Public release of 4th quarter changes will occur on December 10, 2024.

   November 5, 2024
        Added new rule (dup-id-type-check) to ensure there are no duplicate
        @journal-id-type or @pub-id-type values among sibling <journal-id>
        or <pub-id> elements.

   September 10, 2024: Version 5.42
        Public release of 3rd quarter changes.

   September 4, 2024
        In the root template, added $style and $stream values to error message.
        This change will be in the 5.42 release.

   August 12, 2024
        Cut release branch for Version 5.42.
        Public release of 3rd quarter changes will occur on September 10, 2024.

   July 3, 2024
        Added exclude-result-prefixes=#all to stylecheck-helper-templates.xsl to
        suppress xmlns:pmcvar.

   June 25, 2024
        Added "case-study" as an allowed @article-type (in article-type-content-check).

   June 11, 2024: Version 5.41
        Public release of 2nd quarter changes.

   May 29, 2024
        Fixed typo in stylecheck-named-tests.xsl. "If more than on formula" ->
        "If more than one formula".
   
   May 22, 2024
	Tightened the empty-element-check to normalize more whitespace characters 
	and count whitespace-only nodes as empty

   May 13, 2024
	Cut release branch for Version 5.41.
        Public release of 2nd quarter changes will occur on June 11, 2024.

   March 28, 2024
        Removed books from copyright-statement-check_v2.

   March 12, 2024: Version 5.40
        Public release of 1st quarter changes.

   February 14, 2024
        In license-integrity-check, updated 'cc no uri license check' to look for
        text 'CC BY', and to emit an error for manuscripts rather than a warning.

   February 13, 2024
        Cut release branch for Version 5.40.
        Public release of 1st quarter changes will occur on March 12, 2024.

   January 22, 2024
        Added new check (mml-empty-base-or-scripts-check) to ensure that all
        children of msub, msup, msubsup, munder, mover, and munderover contain
        text.

   January 2, 2024
        Added new match template for *[@xml:lang], with a check to ensure @xml:lang
        is a valid ISO 639-1 language code.

   December 12, 2023: Version 5.39
        Public release of 4th quarter changes.

   December 5, 2023
        Multiple updates to web-ext-link-check:
          - @xlink:href allowed to start with http://, https://, ftp://, ftps://,
            www., ftp., or sftp.
          - Fixed typo that was preventing the above test from running.
          - Removed the test that @ext-link-type must be 'uri' when @xlink:href
            contains 'http' or 'https', as that test was never triggered due to
            the conditions under which ext-link-web-check was (and still is) called
            from stylecheck-match-templates.xsl.
		...
		<ERR>
			<xsl:processing-instruction name="SC-DETAILS">
				<xsl:if test="$stream != $style">
					<xsl:text>******* ERROR: $style WAS NOT PASSED CORRECTLY ($style=</xsl:text>
                                        <xsl:value-of select="$style"/>
